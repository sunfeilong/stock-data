...
func TestNextRunDurationZh(t *testing.T) {
    ...
    for i := 0; i < times; i++ {
        ...
        assert.Equal(t, 15, nextTime.Hour())
...
func TestSpecial(t *testing.T) {
    ...
    dateTime := tool.ParseDateTime("2020-06-07 13:00:16")
    durationZh := nextRunDurationZh(dateTime)
    next := dateTime.Add(durationZh)
    assert.Equal(t, "2020-06-07 15:30:00", tool.DateTime(next))

    dateTime = tool.ParseDateTime("2020-06-05 12:09:46")
    durationZh = nextRunDurationZh(dateTime)
    next = dateTime.Add(durationZh)
    assert.Equal(t, "2020-06-05 15:30:00", tool.DateTime(next))

    dateTime = tool.ParseDateTime("2020-08-12 16:34:21")
    durationZh = nextRunDurationZh(dateTime)
    next = dateTime.Add(durationZh)
    assert.Equal(t, "2020-08-13 15:30:00", tool.DateTime(next))
